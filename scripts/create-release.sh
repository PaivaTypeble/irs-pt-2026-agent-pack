...
if gh release view "$TAG" --repo "$GITHUB_REPO" >/dev/null 2>&1; then
  gh release upload "$TAG" "$ZIP_PATH" --repo "$GITHUB_REPO" --clobber >/dev/null
  echo "Updated release $TAG on $GITHUB_REPO"
else
  gh release create "$TAG" "$ZIP_PATH" --repo "$GITHUB_REPO" --title "IRS PT 2026 Agent Pack" --notes "Public release of the IRS PT 2026 agent pack with template zip asset." >/dev/null
